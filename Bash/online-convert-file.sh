...
displayHelp () {
    ...
    echo ""
    echo "This script converts a given File into the target format."
    echo ""
    ...
    echo "* -f|--file            Path to local file which you want to convert"
...
if [ "${targettype}" = "" ] || [ "${targetmethod}" = "" ] || [ "${thisfile}" = "" ] || [ "${notificationurl}" = "" ]; then
    displayHelp
fi
...
optstest=$( echo "${opts}" | sed ':a;N;$!ba;s/\n//g' )

filedata='-s -X POST -F "queue='${optstest}'" -F "file=@'${thisfile}'"'
